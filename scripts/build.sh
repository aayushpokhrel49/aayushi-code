...
main() {
  ...
  # Bundle the Lite XL plugin manager (lpm) next to the executable so that the
  # firstrun plugin can download plugins automatically on the first launch.
  # lpm is a small self-contained binary released by the lite-xl-plugin-manager
  # project, one per target platform.
  if [[ -n "$portable" || "$platform" == "windows" ]]; then
    local lpm_binname="lpm$(get_executable_extension)"
    local lpm_release="${LPM_VERSION:-latest}"
    local lpm_url="https://github.com/lite-xl/lite-xl-plugin-manager/releases/download/${lpm_release}/lpm.${arch}-${platform}$(get_executable_extension)"
    # destinations: portable tree root or windows install root
    local lpm_dest="${build_dir}/${destdir}/lpm$(get_executable_extension)"
    if [[ ! -e "$lpm_dest" ]]; then
      mkdir -p "$(dirname "$lpm_dest")"
      curl --insecure -L -o "$lpm_dest" "$lpm_url"
    fi
    if [[ "$platform" != "windows" ]]; then
      chmod u+x "$lpm_dest"
    fi
    echo "=> Bundled $lpm_binname at $lpm_dest"
  fi
}
