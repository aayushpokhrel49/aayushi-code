...
// reference to lua state for image callbacks
static lua_State *image_lua_state = NULL;
// reference to a table that stores loaded image userdata by integer key
static int IMAGE_REGISTRY_REF = LUA_NOREF;
// next registry key
static int image_registry_idx = 1;
...
static int f_end_frame(UNUSED lua_State *L) {
  ...
  // clear the image registry to release unused references
  lua_newtable(L);
  lua_rawseti(L, LUA_REGISTRYINDEX, IMAGE_REGISTRY_REF);
  image_registry_idx = 1;
  return 0;
}

static RenImage* checkimage(lua_State *L, int idx) {
  RenImage **img = luaL_checkudata(L, idx, API_TYPE_IMAGE);
  return *img;
}

static int f_image_load(lua_State *L) {
  const char *filename = luaL_checkstring(L, 1);
  RenImage *img = ren_image_load(filename);
  if (!img) {
    lua_pushnil(L);
    lua_pushfstring(L, "failed to load image: %s", filename);
    return 2;
  }
  RenImage **userdata = lua_newuserdata(L, sizeof(RenImage *));
  *userdata = img;
  luaL_setmetatable(L, API_TYPE_IMAGE);

  // store in image registry to keep alive during frame
  lua_rawgeti(L, LUA_REGISTRYINDEX, IMAGE_REGISTRY_REF);
  lua_pushvalue(L, -2);
  lua_rawseti(L, -2, image_registry_idx++);
  lua_pop(L, 1);
...
static int f_image_info(lua_State *L) {
  RenImage *img = checkimage(L, 1);
  lua_pushnumber(L, img->width);
  lua_pushnumber(L, img->height);
  return 2;
}

static int f_image_draw(lua_State *L) {
  RenImage *img = checkimage(L, 1);
  int x = luaL_checknumber(L, 2);
  int y = luaL_checknumber(L, 3);
  int w = luaL_checknumber(L, 4);
  int h = luaL_checknumber(L, 5);
  rencache_draw_image(ren_get_target_window(), img, x, y, w, h);
  return 0;
}
...
static const luaL_Reg lib[] = {
  ...
  { "load_image",         f_image_load         },
  { "image_info",         f_image_info         },
  { "draw_image",         f_image_draw         },
  { NULL,                 NULL                 }
};
...
int luaopen_renderer(lua_State *L) {
  ...
  // store lua state for image callbacks
  image_lua_state = L;
  ...
  // image metatable with destructor
  luaL_newmetatable(L, API_TYPE_IMAGE);
  lua_pushcfunction(L, f_image_gc);
  lua_setfield(L, -2, "__gc");
  lua_pop(L, 1);

  // create image registry table
  lua_newtable(L);
  IMAGE_REGISTRY_REF = luaL_ref(L, LUA_REGISTRYINDEX);
