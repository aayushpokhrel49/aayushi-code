...
void rencache_draw_image(RenWindow *window_renderer, void *image, int x, int y, int w, int h) {
  RenRect rect = { x, y, w, h };
  if (w == 0 || h == 0 || !rects_overlap(last_clip_rect, rect)) {
    return;
  }
    ...
    while (next_command(window_renderer, &cmd)) {
      ...
      switch (cmd->type) {
        ...
        case DRAW_IMAGE: {
          ...
          ren_draw_image(&rs, (RenImage*)icmd->image, icmd->rect.x, icmd->rect.y, icmd->rect.width, icmd->rect.height);
          break;
        }
      }
    }
