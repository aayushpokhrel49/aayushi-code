...
--
if not core.status_view:get_item("lsp:diagnostics") then
    core.status_view:add_item({
  predicate = function()
    local dv = get_active_docview()
    if dv and dv.doc.filename then
...
if autocomplete.add_icon then
  ...
  local lsp_symbols_font_path
  for _, dir in ipairs { DATADIR, USERDIR } do
    local candidate = dir .. "/plugins/lsp/fonts/symbols.ttf"
    if system.get_file_info(candidate) then
      lsp_symbols_font_path = candidate
      break
    end
  end
  style.syntax_fonts["lsp_symbols"] = lsp_symbols_font_path
    and renderer.font.load(lsp_symbols_font_path, 15 * SCALE)
    or style.syntax_fonts["normal"]
