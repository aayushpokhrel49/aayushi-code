...
function TreeView:draw_item_icon(item, active, hovered, x, y, w, h)
  ...
  if icon_char and icon_char ~= "" then
    ...
    return icon_font:get_width(icon_char) + self.item_text_spacing
  end
...
function TreeView:draw_item_chevron(item, active, hovered, x, y, w, h)
  if item.type == "dir" then
    -- VS Code uses small triangles for chevrons
    local chevron_icon = item.expanded and "-" or "+"
    local chevron_color = hovered and style.accent or style.dim
...
function TreeView:draw_item_background(item, active, hovered, x, y, w, h)
  if active then
    -- VS Code style: selected item has a subtle background + left accent bar
    local sel_bg = { style.line_highlight[1], style.line_highlight[2], style.line_highlight[3], 200 }
    renderer.draw_rect(x, y, w, h, sel_bg)
    -- Left accent indicator (VS Code blue bar)
    local indicator_w = math.max(2, math.ceil(2 * SCALE))
    local indicator_color = style.tab_active_border or style.caret
    renderer.draw_rect(x, y, indicator_w, h, indicator_color)
  elseif hovered then
    -- VS Code style: hover has very subtle highlight
    local hover_color = { style.line_highlight[1], style.line_highlight[2], style.line_highlight[3], 120 }
...
-- Set treeview visibility based on whether an explicit project is open
if core.empty_project then
  view.visible = false
else
  view.visible = true
end
...
-- Returns the directory context in which new files/folders should be created:
-- * the hovered/selected folder, or the folder containing the hovered/selected file
-- * the project root when right-clicking on empty space
local function get_create_context()
  local item = treeitem()
  if item then
    if item.type == "dir" then
      return item
    ...
    if item.type == "file" then
      local parent_path = common.dirname(item.abs_filename)
      if parent_path then
        return { type = "dir", project = item.project, abs_filename = parent_path,
                 filename = common.basename(parent_path) }
      end
    end
    return item
  end
  ...
  if project then
    return { type = "dir", project = project, abs_filename = project.path,
             filename = common.basename(project.path) }
  end
  return nil
end

function TreeView:on_context_menu(x, y)
  if self.hovered_item then
    ...
    table.insert(items, { text = "Open in System", command = "treeview:open-in-system" })
    if not is_project_folder(self.hovered_item) then
      table.insert(items, ContextMenu.DIVIDER)
      table.insert(items, { text = "Rename", command = "treeview:rename" })
      table.insert(items, { text = "Delete", command = "treeview:delete" })
      table.insert(items, ContextMenu.DIVIDER)
      table.insert(items, { text = "Copy Path", command = "treeview:copy-path" })
      table.insert(items, { text = "Copy Relative Path", command = "treeview:copy-relative-path" })
    end
    if self.hovered_item.type == "dir" then
      table.insert(items, ContextMenu.DIVIDER)
      table.insert(items, { text = "Find in Directory", command = "treeview:search-in-directory" })
      table.insert(items, { text = "Remove directory", command = "treeview:remove-project-directory" })
    end
    return { items = items }, self
  end

  -- Right-click on empty space: offer creation at the project root.
  -- Deselect like VS Code does when clicking empty space, so creation
  -- targets the project root rather than a previously selected folder.
  self.selected_item = nil
  return { items = {
    { text = "New File", command = "treeview:new-file" },
    { text = "New Folder", command = "treeview:new-folder" },
...
command.add(function()
  local item = treeitem()
  return item ~= nil and core.active_view == view, item
end, {
  ["treeview:copy-path"] = function(item)
    system.set_clipboard(item.abs_filename)
    core.log("Copied %s", item.abs_filename)
  end,
  ["treeview:copy-relative-path"] = function(item)
    local rel = item.project:normalize_path(item.abs_filename)
    system.set_clipboard(rel)
    core.log("Copied %s", rel)
  end,
})
