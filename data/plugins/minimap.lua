...
local function get_visible_minline(dv)
  ...
  -- guard against NaN/zero geometry (e.g. transient states during layout)
  if not (y > 0) or not (lh > 0) then return 1 end
  local minline = math.max(0, y / lh + 1)
  if minline ~= minline or minline == math.huge then return 1 end
  return minline
end
...
function MiniMap:get_minimap_lines()
  ...
  if not is_finite(lines_start) or not is_finite(nlines) then
    lines_start, offset, nlines = 1, 0, 1
  end
