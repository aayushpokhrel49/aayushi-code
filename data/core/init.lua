...
local style = require "colors.vscode_dark"
local command
local keymap
local dirwatch
local ime
...
function core.add_project(project)
  ...
  -- legacy lite-xl compatibility: keep project_dir pointing at the root project
  core.project_dir = core.projects[1].path
  core.redraw = true
  return project
end
...
--
-- DATADIR is the location of the installed Aayushi Code Lua code, default color
-- schemes and fonts.
-- USERDIR is the location of the Aayushi Code configuration directory.
--
...
-- Aayushi Code bundles language servers for many languages out of the box:
--   * Python        (pyright)        * C / C++  (clangd)
--   * JavaScript    (tsserver)       * HTML     (html language server)
--   * TypeScript    (tsserver)       * CSS      (css language server)
--   * React JSX/TSX (tsserver)       * JSON     (json language server)
--   * Lua               * Rust       * Go (if `gopls` is installed)
--   * Java (if `jdtls` is installed) * Bash (if `bash-language-server` is installed)
--
-- To disable a bundled language server:
-- config.plugins.lsp_typescript = false
-- config.plugins.lsp_web = false
-- config.plugins.lsp_python = false
-- config.plugins.lsp_c = false
-- config.plugins.lsp_lua = false
-- config.plugins.lsp_rust = false
--
-- To pass custom options to a bundled server, set config.plugins.<name>.config.
-- For example, to disable hints in pyright:
-- config.plugins.lsp_python.config = { settings = { python = { analysis = {
--   typeCheckingMode = "off",
-- } } } }

----------------------------- Keybindings -------------------------------------

-- LSP shortcuts (already enabled by default):
--   ctrl+space      : autocomplete
--   alt+d           : goto definition
--   alt+shift+d     : goto implementation
--   alt+shift+f     : format document
--   alt+r           : rename symbol
--   alt+a           : hover documentation
...
function core.init()
  core.log_items = {}
  core.log_quiet("Aayushi Code version %s - mod-version %s", VERSION, MOD_VERSION_STRING)
  ...
  if project_dir_abs and pcall(core.set_project, project_dir_abs) then
    ...
    pcall(core.set_project, project_dir_abs)
  end
  -- Mark as empty project when no explicit folder or files were given
  ...
  if #plugins_refuse_list.userdir.plugins > 0 or #plugins_refuse_list.datadir.plugins > 0 then
    ...
    core.nag_view:show(
      ...
      string.format(
        ...
        "Please download a recent version from https://github.com/lite-xl/lite-xl-plugins or check Aayushi Code compatibility.",
...
function core.project_absolute_path(path) core.deprecation_log("core.project_absolute_path")
  if not path then return nil end
  if common.is_absolute_path(path) then return common.normalize_path(path) end
  local rp = core.root_project()
  if rp and rp.path then return rp.path .. PATHSEP .. path end
  return (system.absolute_path(".") or "") .. PATHSEP .. path
end
...
function core.compose_window_title(title)
  return (title == "" or title == nil) and "Aayushi Code" or title .. " - Aayushi Code"
end
