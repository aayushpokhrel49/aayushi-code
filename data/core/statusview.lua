...
function StatusView:register_docview_items()
  self:add_item({
    predicate = function() return not core.empty_project end,
    name = "project:info",
    alignment = StatusView.Item.LEFT,
    get_item = function()
      local root = core.root_project()
      local proj_name = root and common.basename(root.path) or "Project"
      return {
        style.accent, style.icon_font, "d",
        style.dim, style.font, " ", style.text, proj_name,
        style.dim, self.separator2
      }
    end,
    command = "core:open-project-folder",
    tooltip = "Click to switch project folder"
  })
